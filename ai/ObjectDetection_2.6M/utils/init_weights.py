...
def initialize_weights(model: nn.Module, image_size: int = 640):
    _initialize_trainable_layers(model)
    _reinit_head_bias(model, image_size)
...
def _reinit_scale_heads(head: nn.Module, image_size=None):
    ...
    for scale_head, stride in zip(heads_list, strides):
        if image_size is None:
            scale_head.init_bias(stride)
        else:
            scale_head.init_bias(stride, img=image_size)

def _reinit_head_bias(model: nn.Module, image_size: int = 640):
